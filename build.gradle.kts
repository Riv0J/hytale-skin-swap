...
// ── Dependency mod auto-build ──────────────────────────────────────────────
// NOTE: these projects must NOT be in includeBuild (settings.gradle.kts) or Gradle
// will file-lock their directories and deadlock when these Exec tasks try to spawn
// a separate gradlew process in the same directory.
// ──────────────────────────────────────────────────────────────────────────
...
afterEvaluate {
    ...
    if (targetTask != null) {
        targetTask.finalizedBy(syncAssets)          // sync assets AFTER server stops
        logger.lifecycle("✅ specific task '${targetTask.name}' hooked for auto-sync and dep-build.")
    } else {
